...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/loopfront726/vapeskid/'..readfile('vapeskid/profiles/commit.txt')..'/'..select(1, path:gsub('vapeskid/', '')), true) end)
...
local vm = loadstring(downloadFile('vapeskid/libraries/vm.lua'), 'vm')()
